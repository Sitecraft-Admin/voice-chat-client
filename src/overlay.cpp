...
static const char* L(const char* th, const char* en, const char* id, const char* filipino) {
    if (g_language == VOICE_LANG_TH) return th;
    if (g_language == VOICE_LANG_ID) return id;
    if (g_language == VOICE_LANG_FILIPINO) return filipino;
    return en;
}

static const char* L(const char* th, const char* en) {
    return L(th, en, en, en);
}
...
void draw_settings_window() {
    ...
    const char* lang_label = (g_language == VOICE_LANG_TH) ? "TH" : (g_language == VOICE_LANG_ID) ? "ID" : (g_language == VOICE_LANG_FILIPINO) ? "PH" : "EN";
